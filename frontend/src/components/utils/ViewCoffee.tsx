...
const ViewCoffee: React.FC<ViewProps> = ({ enviroment }) => {
  return (
      ...
      {enviroment == undefined ? (
        <>
          <Canvas
            ...
            style={{
              backgroundImage:
                "linear-gradient(180deg, #DC5DCB, #748DED)",
            }}
          >
            <Float
            ...
            >
              <Model position={[0, -3, -10]} rotation={[0, -Math.PI / 2, 0]} />
        ...
        <Canvas camera={{ position: [0, 0, 2], fov: 70 }} shadows={true}>
          ...
          >
            <Model position={[0, -3, -10]} rotation={[0, -Math.PI / 2, 0]} />
            <directionalLight position={[3, 0, 5]} intensity={1.5} />
            <Environment
              preset={enviroment}
              background
