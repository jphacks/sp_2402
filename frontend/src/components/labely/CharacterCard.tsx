...
const CharacterCard: React.FC<CharacterCardProps> = ({ CharacterCardLevel, CharacterCardname }) => {
    // 親密度に応じてアイコンを選択
    const iconPath =
    ...
    return (
        <div className={styles.CharacterCardContainer}>
            <img src="/labely/CharacterCard/dog.svg" alt="Character Icon" className={styles.backgroundIcon} />
            <div className={styles.CharacterCardLevel}>{`${CharacterCardLevel}%`}</div> {/* 親密度を表示 */}
            ...
            <img src={iconPath} alt={`${CharacterCardLevel}%`} className={styles.CharacterCardIcon} />
